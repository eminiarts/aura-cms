...
The `process` backend requires a POSIX `/bin/sh`; an enumerable `/proc/self/fd` or `/dev/fd`; PHP's `proc_open`, `pcntl` signal handlers and signal masking, and `posix_kill` in the request runtime; a CLI PHP with fork/exec/wait and POSIX signal support; and a readable Artisan entry point. `worker_php: null` uses Symfony's SAPI-aware PHP executable finder, so an FPM binary is never reused as the worker command; set an absolute path when PHP CLI lives in a non-standard location. `worker_artisan: null` resolves to `base_path('artisan')`; set an absolute path only for a non-standard application layout. Unsupported request runtimes fail before a supervisor is launched, and unsupported CLI runtimes fail before the application worker is launched.

The small CLI supervisor gives every operation its own monotonic deadline and watches both the request parent and its own parent. Termination signals remain blocked while watcher and worker PIDs are created and published; handlers are installed before either fork, then signals are unblocked only after the responsible parent can kill and reap its child. It kills and reaps the application worker if either parent disappears, including `SIGKILL`, or when the deadline expires. The application worker starts with PHP process-spawn, process-group, signalling, and FFI functions disabled, so hooks and adapters cannot fork, exec, call `setsid`, signal the parent, or daemonize descendants. The request runtime also kills active supervisors on output overflow, timeout, `SIGTERM`/`SIGINT`, and fatal shutdown. If any containment prerequisite is unavailable, `execution_backend` is `none`, or configuration is invalid, the complete search fails closed. `inline-testing` exists only for package unit tests and is rejected outside Laravel's unit-test runtime.
