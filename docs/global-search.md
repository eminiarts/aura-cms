...
The total deadline includes resource discovery. After class validation and resource resolution, Aura evaluates `viewAny` before any resource-controlled discovery, field, query, visibility, adapter, or presentation hook. The resource deadline includes those authorized hooks, record policies, title dependencies, and destinations. Timeout or malformed worker output is discarded. `isolated_payload_bytes` bounds worker output. Exceptions and deadlines are logged using only resource class, reason, exception class, and configured timeout; search terms and result data are never included.
