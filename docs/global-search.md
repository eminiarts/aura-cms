...
Query limits are installed before the worker reloads the authenticated user and are enforced with a before-execution callback on every Laravel-managed database connection created by the worker. Guarded connection objects are tracked by weak object identity, so purging and recreating a connection installs a fresh callback even when PHP reuses the previous numeric object ID, without retaining discarded connections. Authentication plus queries issued by policies, hooks, title presentation, and custom adapters count even when those extensions do not cooperate with `GlobalSearchBudget`. A timed-out or malformed resource pessimistically consumes its remaining per-resource query allocation in the parent.
