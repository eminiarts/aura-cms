...
User rows are written under the canonical physical name `u{compact-owner-identity}{option}`. The compact owner identity is the 14-character result of `VersionedCache::compactIdentity('option.user.owner', $userId)`. Because compact identities can theoretically collide, the separate `owner_identity` column stores the full SHA-256 result of `VersionedCache::identity('option.user.owner', $userId)` and is verified before a row is read, changed, or claimed.

`aura-user-option-v2:{owner-hash}:{option}` and, for eligible integer-key users, `user.{id}.{option}` are readable migration aliases only; neither is the canonical write target. An update found through either alias renames the row to the compact canonical name, records the full `owner_identity`, and removes redundant aliases. Ambiguous legacy string-key rows are deliberately not auto-claimed. Scoped option names are unique in the database; the upgrade migration refuses to guess when pre-existing duplicates need operator resolution. Options are soft deletable, and helper writes restore the existing identity instead of inserting a duplicate.
