...
- `getFields()` must declare `: array` on every resource (`DefinesFields` does). Field definitions without `type` or `slug` throw at boot.
- The GET login route is named `login` (was `aura.login`); `/admin` redirects straight to `/login`.
- Resource forms persist only declared input fields. Code that relied on extra keys in `form.fields` being mass-assigned must declare a field or a `set{Slug}Field` method.
- Custom-table resources generated by `aura:resource --custom` set `$usesMeta = false`. Existing custom-table resources that want column storage must add it.
