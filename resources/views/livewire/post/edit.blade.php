<div>
    ...
    <x-breadcrumbs>
        ...
        <x-breadcrumbs.li :href="route('post.index', $slug)" :title="Str::plural($slug)" />
        <x-breadcrumbs.li :title="$model->title" />
    </x-breadcrumbs>
    @endif
    ...
    <div class="mb-4">
        <x-fields.wrapper :field="['slug' => 'title']" wrapperClass="" class="-mx-4">
            <x-input.text label="Title" wire:model.defer="post.title" error="post.title" placeholder="Title">
            </x-input.text>
        </x-fields.wrapper>
    </div>
    @endif
