    ...
    <div x-show="open" class="my-10">
        ...
        <div x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true"
            ...
            <div class="flex justify-center items-center h-40 text-center text-gray-500 bg-white rounded-sm border border-gray-900 border-dashed cursor-pointer">
                <div class="flex justify-center items-center w-full" nonce="{{ csp_nonce() }}" style=""x-on:click="$refs.fileInput.click()"> Drag and drop to upload <br> or <br> Select Files </div>
        ...
        <div class="pt-1" >
            <ul role="list" class="flex relative">
                ...
                <div class="my-3 rounded-sm transition-all duration-500" nonce="{{ csp_nonce() }}" style="" wire:key="{{$loop->index}}">
            {{-- GRID VIEW --}}
            ...
            {{-- END LIST VIEW --}}
                    ...
                    <div class="absolute pt-8 pb-8 w-full">
                        <button wire:loading.remove wire:click.prevent="save" class="pt-1 pr-2 pb-2 pl-2 w-full text-gray-500 bg-white rounded-sm border" nonce="{{ csp_nonce() }}" style="">{{ __('Save') }}</button>
                        <button wire:loading wire:target="save" class="p-2 w-full text-gray-500 bg-white rounded-sm border" nonce="{{ csp_nonce() }}" style=""
