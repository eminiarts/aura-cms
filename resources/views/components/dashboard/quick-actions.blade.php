...
<div {{ $attributes->merge(['class' => trim("{$colSpan} rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-950/10 dark:ring-white/10")]) }}>
    <div class="p-6">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Quick Actions') }}</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Common tasks and actions') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-1 mt-4 -mx-3 sm:grid-cols-2">
            @if (config('aura.features.create_resource'))
                <button onclick="Livewire.dispatch('openModal', { component: 'aura::create-resource' })"
                    class="flex gap-3 items-center px-3 py-2.5 text-left rounded-lg transition group hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <div class="p-2 bg-green-50 rounded-lg shrink-0 dark:bg-green-900/50 group-hover:bg-green-100 dark:group-hover:bg-green-900/70">
                        <svg class="text-green-600 size-5 dark:text-green-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" color="currentColor" fill="none">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Create Resource') }}</span>
                        <span class="text-xs text-gray-500 truncate dark:text-gray-400">{{ __('Add a new content type') }}</span>
                    </div>
                </button>
            @endif

            <a href="{{ route('aura.settings') }}" wire:navigate class="flex gap-3 items-center px-3 py-2.5 rounded-lg transition group hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <div class="p-2 bg-pink-50 rounded-lg shrink-0 dark:bg-pink-900/50 group-hover:bg-pink-100 dark:group-hover:bg-pink-900/70">
                    <svg class="text-pink-600 size-5 dark:text-pink-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" color="currentColor" fill="none">
                ...
                <div class="flex flex-col min-w-0">
                    <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Settings') }}</span>
                    <span class="text-xs text-gray-500 truncate dark:text-gray-400">{{ __('Configure your site') }}</span>
                </div>
            </a>

            @if ($currentTeamEditUrl)
                <a href="{{ $currentTeamEditUrl }}" wire:navigate class="flex gap-3 items-center px-3 py-2.5 rounded-lg transition group hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Edit Team') }}</span>
                        <span class="text-xs text-gray-500 truncate dark:text-gray-400">{{ __('Manage team settings') }}</span>
                    </div>
                </a>
            @endif

            @if (config('aura.features.plugins'))
                <a href="{{ route('aura.plugins') }}" wire:navigate class="flex gap-3 items-center px-3 py-2.5 rounded-lg transition group hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <div class="p-2 bg-purple-50 rounded-lg shrink-0 dark:bg-purple-900/50 group-hover:bg-purple-100 dark:group-hover:bg-purple-900/70">
                        <svg class="text-purple-600 size-5 dark:text-purple-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" color="currentColor" fill="none">
                    ...
                    <div class="flex flex-col min-w-0">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Plugins') }}</span>
                        <span class="text-xs text-gray-500 truncate dark:text-gray-400">{{ __('Browse and manage plugins') }}</span>
                    </div>
                </a>
            @endif
        </div>
