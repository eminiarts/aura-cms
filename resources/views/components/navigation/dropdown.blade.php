        ...
        'bg-primary-800/70 rounded-lg': expanded,
          ...
          >
              ...
              <span x-cloak x-show="expanded" aria-hidden="true" class="ml-4">
                <x-icon.chevron-up class="w-5 h-5 text-white/60" />
              </span>
              <span x-cloak x-show="!expanded" aria-hidden="true" class="ml-4">
                <x-icon.chevron-down class="w-5 h-5 text-white/60" />
              </span>
          </button>

        <div x-show="expanded" x-cloak class=" p-2" x-collapse>
            {{$slot}}
        </div>
    </div>
...
<div x-data="{ active: {{ (Request::fullUrlIs($route ? route($route, $id) : '') ? ' 1' : '0')  }}, compact: {{ $compact ? '1' : '0' }} }" class="">
    <div x-data="{
        ...
        set expanded(value) {
          ...
          >
              ...
              <span x-show="expanded" x-cloak aria-hidden="true" class="ml-4">
                <x-icon.chevron-up class="w-6 h-6" />
              </span>
              <span x-cloak x-show="!expanded" aria-hidden="true" class="ml-4">
        ...
        <div x-show="expanded" x-cloak class="pl-3 ml-[1.4rem] border-l-2 border-l-primary-600" x-collapse>
            {{$slot}}
        </div>
    </div>
...
<div x-data="{ active: {{ (Request::fullUrlIs($route ? route($route, $id) : '') ? ' 1' : '0')  }}, compact: {{ $compact ? '1' : '0' }} }" class="">
    <div x-data="{
        ...
        <div x-show="expanded" x-ref="container" x-cloak class="pl-1 ml-[1.4rem] border-l-2 border-l-gray-600" x-collapse>
            {{$slot}}
        </div>
    </div>
