<x-aura::fields.wrapper :field="$field">
    @if (optional(optional($field)['options'])['native'])
        <div class="relative right-4 left-4 bottom-6 z-40 md:relative md:inset-0"
            x-data="{
                color: $wire.entangle('resource.fields.{{ optional($field)['slug'] }}'),
                init () {
        ...
        >
            <x-aura::input.text :disabled="$field['field']->isDisabled($this->resource, $field)" x-model="color" error="post.fields.{{ optional($field)['slug'] }}" placeholder="{{ optional($field)['placeholder'] ?? optional($field)['name'] }}" id="post-field-{{ optional($field)['slug'] }}" autocomplete="{{ optional($field)['autocomplete'] ?? '' }}"></x-aura::input.text>
    ...
    <div wire:ignore class="flex items-center"
        x-data="{
            selectedColor: $wire.entangle('resource.fields.{{ optional($field)['slug'] }}'),
            init() {
    ...
    >
        ...
        <div class="ml-2">
            <x-aura::input.text :disabled="$field['field']->isDisabled($this->resource, $field)" x-model="selectedColor" error="post.fields.{{ optional($field)['slug'] }}" placeholder="{{ optional($field)['placeholder'] ?? optional($field)['name'] }}" id="post-field-{{ optional($field)['slug'] }}"></x-aura::input.text>
        </div>
    </div>
