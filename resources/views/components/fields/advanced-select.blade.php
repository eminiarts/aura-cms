@php
    $api = $field['api'] ?? ($field['field']->api ?? false);
    ...
    if ($api) {
        $values = [];
        $selected = isset($this) ? ($this->form['fields'][$field['slug']] ?? null) : ($form['fields'][$field['slug']] ?? null);
        $selectedValues = $field['field']->selectedValues($field['resource'], $selected, $field);
    } else {
...
<x-aura::fields.wrapper :field="$field">
    ...
    <div wire:ignore class="w-full" x-data="{
        ...
        async fetchApi(append = false) {
            ...
            try {
                const response = await fetch(@js(route('aura.api.fields.values')), {
                    method: 'POST',
                    credentials: 'include',
                    headers: {
        ...
        toggleListbox() {
            ...
            if (this.showListbox && this.api && !this.loaded) {
                this.fetchApi();
            }
            ...
            <template x-if="showListbox">
                <ul x-ref="listbox" x-transition.origin.top x-cloak
                    ...
                    <template x-if="loading">
                        <li role="status" class="px-4 py-3 text-sm text-gray-500">{{ __('Loading...') }}</li>
                    </template>
                    <template x-if="loadError">
                        <li class="px-4 py-3 text-sm">
                            <span role="alert">{{ __('Unable to load options.') }}</span>
                            <button type="button" @click="fetchApi()" class="underline">{{ __('Retry') }}</button>
                        </li>
                    </template>
                    <template x-if="!loading && !loadError && filteredItems.length === 0">
                        <li class="px-4 py-3 text-sm text-gray-500">{{ __('No results found.') }}</li>
                    </template>
                    <template x-if="!loading">
                    ...
                    <template x-if="api && hasMore && !loading && !loadError">
                        <li>
                            <button type="button" @click="fetchApi(true)" class="px-4 py-3 w-full text-sm text-primary-600">{{ __('Load more') }}</button>
                        </li>
                    </template>
                </ul>
            </template>
