...
<button type="button" wire:click="addField('{{ $id }}', '{{ $slug }}', '{{ addslashes($type) }}', '{{ $children }}', '{{ $model }}')" class="resource-editor-add-field-{{ $slug }} flex relative justify-center items-center p-2 w-full text-center rounded-lg border-2 border-dashed border-gray-400/30 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
    <div class="mr-2">
