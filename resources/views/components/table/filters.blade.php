...
<div>
    ...
    <div>
        ...
        <div class="flex flex-col space-y-2">
            ...
            <div class="flex items-center">
                <x-aura::input.checkbox wire:model.live="filters.taxonomy.{{ $taxonomy->taxonomy }}" name="taxonomy_{{ $taxonomy->id }}" id="taxonomy_{{ $taxonomy->id }}" :label="$taxonomy->name" :value="$taxonomy->id" />
            </div>
            @endforeach
        </div>
    </div>
    ...
    <p class="block font-semibold">{{ __('Custom Filters') }}</p>
     ...
     {{-- @dump( $filters) --}}
        ...
        <x-aura::dialog-modal wire:model.live="showSaveFilterModal">
            <x-slot name="title">
                {{ __('Save Filter') }}
            </x-slot>
