...
>
    ...
    <button
        ...
        class="{{ $label ? 'ml-3' : '' }} relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900"
    >
        <span
    ...
    <label
        @click="$refs.toggle.click(); $refs.toggle.focus()"
        :id="$id('toggle-label')"
        class="ml-3 text-sm text-gray-700 transition-colors cursor-pointer dark:text-gray-300"
    >
      {{ $labelAfter }}
    </label>
    @endif
