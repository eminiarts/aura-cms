...
<button type="button" wire:click="addField({{ $id }}, '{{ $slug }}', '{{ addslashes($type) }}', '{{ $children }}')" class="relative flex items-center justify-center w-full p-2 text-center border-2 border-gray-400/30 border-dashed rounded-lg hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
    <div class="mr-2">
