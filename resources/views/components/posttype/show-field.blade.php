...
<div>
  ...
  @if (optional($field)['field']->group)
    <div class="aura-card">
      <div class="flex justify-between mb-4 flex-start">
        ...
        <div>
          ...
          <x-tippy text="Edit field">
            <x-button.border wire:click="$emit('openSlideOver', 'edit-field', {{ json_encode(['fieldSlug' => $field['slug'], 'slug' => $slug, 'field' => $this->sendField($field['slug'])]) }})">
      ...
      @if (isset($field['fields']))
        <div class="flex flex-wrap items-start -mx-4 draggable-container">
          @foreach($field['fields'] as $key => $f)
            ...
            >
              <x-posttype.show-field :field="$f" :slug="$slug" />
            </div>
            @if ($loop->last)
              @if ($f['type'] == 'App\Aura\Fields\Repeater')
              @elseif ($f['type'] == 'App\Aura\Fields\Tab')
                <div class="w-full px-4">
                  <x-posttype.add-field :id="$f['_id']" :slug="$f['slug']" :type="$f['type']" :children="isset($f['fields']) ? count($f['fields']) : 0"/>
                </div>
              @elseif ($f['type'] == 'App\Aura\Fields\Panel')
                <div class="w-full px-4">
                  <x-posttype.add-field :id="$f['_id']" :slug="$f['slug']" :type="$f['type']" />
                </div>
              @else
                <div class="w-full px-4">
                  <x-posttype.add-field :id="$f['_id']" :slug="$f['slug']" :type="$f['type']" />
    ...
    @else
        ...
        <div class="flex items-center justify-between mb-4 aura-card-small">
          <div class="mt-1 mr-2 draggable-handle">
            <svg class="w-5 h-5 text-gray-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 9L2 12M2 12L5 15M2 12H22M9 5L12 2M12 2L15 5M12 2V22M15 19L12 22M12 22L9 19M19 9L22 12M22 12L19 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </div>
          ...
          <div class="flex space-x-2">
              <x-tippy text="Duplicate">
                <x-button.border size="xs" wire:click="duplicateField({{ $field['_id'] }}, '{{ $field['slug'] }}')">
              ...
              <x-tippy text="Edit field">
                <x-button.border size="xs" wire:click="$emit('openSlideOver', 'edit-field', {{ json_encode(['fieldSlug' => $field['slug'], 'slug' => $slug, 'field' => $this->sendField($field['slug'])]) }})">
