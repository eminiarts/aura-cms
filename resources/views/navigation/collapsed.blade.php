    ...
    @foreach(app('aura')::navigation() as $group => $resources)
      @foreach($resources as $resource)
          ...
          @if (isset($resource['dropdown']) && $resource['dropdown'] !== false)
              <x-aura::navigation.dropdown-icon>
                  ...
                  @foreach($resource['items'] as $resource)
                      <x-aura::navigation.item-dropdown route="aura.resource.index" :id="$resource['type']" :strict="false">
                          <div class="{{ $iconClass }}">
                              {!! $resource['icon'] !!}
                          </div>
          ...
          @else
              ...
              <x-aura::navigation.item-icon route="aura.resource.index" :id="$resource['type']" :tooltip="$resource['name']" :strict="false">
                <div class="{{ $iconClass }}">
                  {!! $resource['icon'] !!}
                </div>
              </x-aura::navigation.item-icon>
