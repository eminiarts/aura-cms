...
use Aura\Base\Tests\Fixtures\GlobalSearchProcessUser;
...
function registerHardeningSearchResources(array $resources): void
{
    ...
    Aura::fake();
    config(['aura.resources' => $configuredResources]);
...
INSERT INTO roles (id, slug, super_admin, permissions, team_id)
VALUES (1, 'admin', 1, '[]', NULL);
INSERT INTO user_role (user_id, role_id, team_id)
VALUES (1, 1, 11);
...
SQL);
    ...
    config([
        ...
        'aura.resources.user' => GlobalSearchProcessUser::class,
        'aura.teams' => true,
        'auth.providers.users.model' => GlobalSearchProcessUser::class,
    ...
    $user = GlobalSearchProcessUser::on('process_search')->withoutGlobalScopes()->findOrFail(1);
...
function configureCollidingFreshProcessSearchHarness(): array
{
    ...
    foreach ([
        ...
        $pdo->exec('CREATE TABLE roles (id INTEGER PRIMARY KEY, slug TEXT NOT NULL, super_admin INTEGER NOT NULL DEFAULT 0, permissions TEXT, team_id INTEGER)');
        $pdo->exec('CREATE TABLE user_role (user_id INTEGER NOT NULL, role_id INTEGER NOT NULL, team_id INTEGER)');
        ...
        $pdo->exec("INSERT INTO roles (id, slug, super_admin, permissions, team_id) VALUES (1, 'admin', 1, '[]', NULL)");
        $pdo->exec('INSERT INTO user_role (user_id, role_id, team_id) VALUES (1, 1, 11)');
    ...
    return [
        ...
        'marker' => $markerPath,
        'user' => GlobalSearchProcessUser::on('process_search_tenant_a')->withoutGlobalScopes()->findOrFail(1),
...
test('the inline default adapter seals visibility scopes and its candidate limit after callbacks', function () {
    ...
    HardeningBeforeQueryMutationResource::createForTeamForSystem($otherTeam->getKey(), [
        'title' => 'Cross Tenant Callback Needle',
    ]);

    foreach (range(1, 6) as $index) {
        HardeningBeforeQueryMutationResource::createForTeamForSystem($this->searchUser->current_team_id, [
...
test('the inline default adapter rejects union callbacks instead of leaking another tenant', function () {
    ...
    HardeningUnionBeforeQueryResource::createForTeamForSystem($this->searchUser->current_team_id, [
        'title' => 'Current Tenant Union Needle',
    ]);
    HardeningUnionBeforeQueryResource::createForTeamForSystem(Team::factory()->createQuietly()->getKey(), [
...
test('the inline default adapter validates the final visibility pass', function () {
    ...
    HardeningSecondPassUnionResource::createForTeamForSystem($this->searchUser->current_team_id, [
        'title' => 'Current Tenant Second Pass Needle',
    ]);
    HardeningSecondPassUnionResource::createForTeamForSystem(Team::factory()->createQuietly()->getKey(), [
...
test('the inline default adapter preserves first pass visibility predicates and bindings', function () {
    ...
    HardeningSecondPassPredicateErasingResource::createForTeamForSystem($this->searchUser->current_team_id, [
        'title' => 'Current Tenant Predicate Needle',
    ]);
    HardeningSecondPassPredicateErasingResource::createForTeamForSystem(Team::factory()->createQuietly()->getKey(), [
...
test('teams mode fails closed without a current team unless a resource explicitly opts in', function () {
    ...
    HardeningUnscopedResource::createForTeamForSystem($team->getKey(), [
        'type' => HardeningUnscopedResource::getType(),
        'title' => 'Null Team Hidden Needle',
    ]);
    HardeningTrustedUnscopedResource::createForTeamForSystem($team->getKey(), [
...
test('candidate search never eager loads unrelated meta rows', function () {
    ...
    $retrievedMeta = 0;
    $metaQueries = 0;
    $resourceMorphClass = $result->getMorphClass();
    Event::listen('eloquent.retrieved: '.Meta::class, function () use (&$retrievedMeta): void {
        $retrievedMeta++;
    });
    DB::listen(function ($query) use (&$metaQueries, $resourceMorphClass): void {
        if (str_contains($query->sql, 'from "meta"')
            && in_array($resourceMorphClass, $query->bindings, true)) {
            $metaQueries++;
        }
    });
...
test('declared title meta dependencies load only their retained keys', function () {
    ...
    $metaSql = [];
    $resourceMorphClass = $result->getMorphClass();
    DB::listen(function ($query) use (&$metaSql, $resourceMorphClass): void {
        if (str_contains($query->sql, 'from "meta"')
            && in_array($resourceMorphClass, $query->bindings, true)) {
...
PHP;
    $process = new Process([PHP_BINARY, '-r', $source, dirname(__DIR__, 2).'/vendor/autoload.php']);
    $process->run();
...
test('unicode matching has the same semantics on mysql when a local service is available', function () {
    ...
    try {
        Schema::connection('global_search_mysql')->create('users', function (Blueprint $table): void {
            $table->id();
            $table->string('name');
            $table->string('email');
        ...
        Schema::connection('global_search_mysql')->create('meta', function (Blueprint $table): void {
            $table->id();
        ...
        Schema::connection('global_search_mysql')->create('roles', function (Blueprint $table): void {
            $table->id();
            $table->string('slug');
        ...
        Schema::connection('global_search_mysql')->create('user_role', function (Blueprint $table): void {
        ...
        $mysqlConnection->table('users')->insert([
            'id' => 1,
            'name' => 'MySQL Search User',
            'email' => 'mysql-search@example.test',
            'password' => 'unused',
            'current_team_id' => null,
            'global_admin' => true,
        ]);
        $mysqlConnection->table('roles')->insert([
            'id' => 1,
            'slug' => 'admin',
            'super_admin' => true,
            'permissions' => '[]',
        ...
        $mysqlConnection->table('user_role')->insert([
            'user_id' => 1,
            'role_id' => 1,
            'team_id' => null,
        ]);
        ...
        $this->actingAs(
            GlobalSearchProcessUser::on('global_search_mysql')->withoutGlobalScopes()->findOrFail(1),
        );

        registerHardeningSearchResources([HardeningMysqlUnicodeResource::class]);
        Gate::policy(HardeningMysqlUnicodeResource::class, HardeningSearchPolicy::class);
