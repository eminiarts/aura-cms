...
class GlobalSearch extends Component
{
    ...
    {
        $this->refreshBookmarks($this->authorizeUse());
    }
    ...
    {
        try {
            if (! $this->usesConfiguredUserProvider($user)
    ...
    private function authorizeUse(): User
    {
        ...
        if (! $user instanceof User || ! $this->usesConfiguredUserProvider($user)) {
            abort(403);
        }
    ...
    private function refreshBookmarks(User $user): void
    {
        $this->bookmarks = $user->getOptionBookmarks();
    }
    ...
    private function usesConfiguredUserProvider(mixed $user): bool
    {
        $userClass = config('aura.resources.user');

        return is_string($userClass)
            && is_a($userClass, User::class, true)
            && $user instanceof $userClass;
    }
