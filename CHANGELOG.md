...
- Hardened embedded-resource migration ownership for concurrent installs with atomic claims and compare-and-swap transitions, exact ordered-index validation, retryable interrupted states, and ownership proof isolated from runtime incarnation queries; legacy proof artifacts are cleaned up without deleting host data.
- Preserved compatibility with custom fields that override `rendersOnIndex()` without a return type while normalizing the adapter result to `bool`.
