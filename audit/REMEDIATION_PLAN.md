...
- **Changes (as landed):**
  ...
  5. Gate **dependabot auto-merge** on required status checks (`PROC-04`): branch protection on `main` now requires the serial test check `P8.3 - prefer-stable - ubuntu-latest` (none existed — auto-merge was unconditional). After PR-1 merges, also require the new parallel check.
  6. PHPStan (`PROC-05`): the workflow had been **manually disabled** in repo settings — re-enabled; baseline committed so CI fails on **new** errors only; runs on PRs and via dispatch. Burning down the baseline is later, per-area.
  7. CI test + PHPStan jobs run on **PHP 8.3** (not 8.2): the harness needs `pest-plugin-livewire` v4 → Livewire 4, the stack local dev runs and PR-5 commits to; on 8.2 the resolver falls back to Livewire 3 and the suite/analysis fail on LW-01..04 mismatches.
  8. Fixes the new CI caught on its first runs: Composer ≥2.9 advisory blocking made every install fail (`config.policy.advisories.block: false`, applies to the root package only); `pest-plugin-livewire ^4.0` contradicted the `php ^8.2` floor (widened to `^3.0|^4.0`); `Fields\DateTime` class-name case in `CreateResourceMigration` only autoloaded on macOS (4 tests failed on Linux); serial job needs `pest --no-coverage` (Pest 4 aborts when coverage reports are configured without a driver).
- **Tests/validation:** 8 consecutive local `pest --parallel` full-suite runs and the CI jobs themselves; remaining red is exactly the 3 known deterministic failures owned by PR-5/PR-6.
- **Risk:** medium (CI config + test-support); the only runtime change is the `Fields\Datetime` case fix.
- **Depends on:** PR-0.
- **State:** ✅ open as PR #23.
